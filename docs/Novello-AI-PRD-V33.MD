...
| Theme System: 9 themes with live switching | ✅ Complete with theme-aware Agent Avatars | 
...
- **Storage:** Firebase Storage (Local FS fallback)
- **Functions:** Firebase Cloud Functions / Next.js API Routes
- **Background Workers:** BullMQ + Redis + Node.js Scripts (Audio/Voice Cloning)
- **AI Core:** Vercel AI SDK + Ollama (Local/OSS)
...
- **Design System — "Skeuomorphic Publisher"**: The default aesthetic is now deeply textured, emulating a physical publishing house editor's desk. Features ivory paper backgrounds, embossed buttons, and tightly cropped Agent AI Avatars (object-position center 25%) on a 5:4 aspect-ratio banner.
...
| Version | Description |
|---|---|
| **V33 (CURRENT)** | ADDED: Skeuomorphic Publisher UI Redesign, Agent Avatar specific cropping, "All-Ollama" local inference logic (deprecating cloud LLMs), and a patched Write Node Stream Decoder to resolve TipTap prose crashes. |
